...
const _defs = [
  ...
  // 陳列用の専用パンブロック（ショーケースに並べると品揃えが伝わる）。
  block('baguette_disp', 'バゲット', 60, 0xcd9646, '細長く焼いたフランスパン。ショーケースに並べて。'),
  block('campagne_disp', 'カンパーニュ', 61, 0xc68a42, '丸い天然酵母のハードパン。看板商品。'),
  block('pastry_disp', 'クロワッサン', 62, 0xd6a856, 'バターを折り込んだ層のあるペストリー。'),
  // 店構えサイン（店舗の認知をつくる）。
  block('open_sign', '営業中サイン', 65, 0x5c6b4a, '「OPEN」を知らせる営業中の看板。入口に。'),
  block('shop_sign', '店名サイン', 63, 0xf5ede4, 'プチヘルメースの切手モチーフ看板。ファサードに掲げて。'),
  block('aframe', 'A型黒板', 64, 0x2c2e30, '店先の手書き黒板。本日のおすすめを。'),

  // --- 旧南方小学校 schoolhouse blocks (S1) ---
  block('shoe_cubby', '下駄箱', 66, 0x96744a, '昇降口の下駄箱。3×3のます目に上履きがちょこん。'),
  block('green_board', '緑の黒板', 67, 0x184e3a, 'チョーク受け付きの緑黒板。今日の献立やメニューを。'),
  block('school_floor', '教室の木床', 68, 0xc6a876, '日に焼けた明るい教室の木床。歩くとなつかしい音。'),
  block('plaster', 'しっくい壁', 69, 0xf5ede4, '暖かみのある白壁。校舎リノベの基本材。'),
  block('sash_window', '窓枠ガラス', 70, 0xd8ecf2, '白い桟の入った校舎の窓。光をたっぷり取り込む。'),
  block('gym_floor', '体育館の床', 71, 0xd0b27a, 'コートラインが引かれた体育館のフローリング。'),
  block('school_clock', '校舎の時計', 72, 0xf2f0e8, '校舎正面の丸時計。開店は11時30分。'),
  block('school_emblem', '校章', 73, 0x28325c, '紺地に金のロゼットが輝く旧南方小学校の校章。'),
  block('notice_board', '掲示板', 74, 0xbc9460, 'コルクの掲示板。お知らせや作品を貼り出そう。'),
  block('sink_unit', '手洗い場', 75, 0xc6cace, '廊下のステンレス手洗い場。給食の前は手を洗おう。'),
  block('vault_box', '跳び箱', 76, 0xc49e68, '体育館の跳び箱。何段まで跳べるかな。'),
  block('sakura_leaves', '桜の葉', 77, 0xebaac3, '満開の桜。校庭をやさしい桜色に染める。'),
  block('cedar_log', 'スギの原木', 78, 0x704a30, '里山に育つスギの幹。まっすぐで力強い。'),
  block('cedar_leaves', 'スギの葉', 79, 0x26483a, 'スギの濃い青緑の葉。里山の杉林をつくる。'),
  block('vending', '自動販売機', 80, 0x2658a8, '明かりのともる自販機。冷たい飲み物をどうぞ。'),
  block('rice', '稲', 81, 0x78b246, '田んぼに揺れる稲。実りの秋が楽しみ。'),
  block('tin_roof', 'トタン屋根', 82, 0x969ea2, '波板のトタン屋根。雨音がよく響く。'),
  block('kawara', '瓦', 83, 0x5c6a7a, '青灰色の日本瓦。重なる弧が美しい。'),
  block('pain_de_mie_disp', '食パン', 84, 0xf0e4c4, 'ふんわり焼き上げた食パン。陳列棚の人気者。'),
  block('tartine_disp', 'タルティーヌ', 85, 0xe8d5a8, '日替わりタルティーヌのディスプレイ。彩りが映える。'),
  block('soup_pot', 'スープ鍋', 86, 0xe08830, '規格外野菜たっぷりのスープがことこと煮える鍋。'),
  block('curry_pot', 'カレー鍋', 87, 0x9a5a24, 'スパイスの香りが立ちのぼるカレーの寸胴鍋。'),
  block('quiche_disp', 'キッシュ', 88, 0xe0a84a, '黄金色に焼けたキッシュ。カットして提供。'),
  block('cookie_tray', '焼き菓子トレー', 89, 0xcaa050, '酵母ビスコッティやクッキーを並べたトレー。'),
  block('slat_shelf', '木の陳列棚', 90, 0xae8858, '木のスラットを重ねた陳列棚。パンを面で見せる。'),
  block('basket_bread', 'パンかご', 91, 0xa0763e, '編みかごに盛られた焼きたてパン。粉の香りつき。'),
  block('price_card', '値札カード', 92, 0xf5edd8, 'クリーム色の値札カード。赤い¥が目印。'),
  block('coffee_kit', 'コーヒー器具', 93, 0xe2d8c6, 'ドリッパーとサーバー。淹れたての一杯を。'),
  block('menu_stand', 'メニュースタンド', 94, 0x46341e, '本日のメニューを掲げるスタンド。'),
  block('stainless', '調理台', 95, 0xbcc0c6, '旧給食室のステンレス調理台。ピカピカに磨いて。'),
  block('school_desk', '学校の机', 96, 0xcaa876, '町産木材の学校机。天板のキズも思い出。'),
  block('school_chair', '学校の椅子', 97, 0xc49e68, '木の学校椅子。机とセットでどうぞ。'),
  block('flag', '国旗', 98, 0xf0f0f2, '校庭のポールにはためく旗。'),
  block('guard_rail', 'ガードレール', 99, 0xeef0f0, '道路沿いの白いガードレール。'),
  block('brand_green', 'ブランドグリーン', 100, 0x5c6b4a, 'プチヘルメースの深緑 #5C6B4A。お店の顔になる色。'),
  block('wheat_beige', '麦色ブロック', 101, 0xe8d5b7, 'やさしい麦色 #E8D5B7。深緑と好相性。'),
  block('compost', 'ぐるぐるコンポスト', 102, 0x6a4a2c, 'パンくずや生ごみが堆肥に変わる木枠のコンポスト。'),
  block('yeast_shelf', '酵母瓶の棚', 103, 0xc88840, 'いちご・柿・ゆず・ハーブの酵母瓶がプクプク並ぶ棚。'),
  block('isshou_pan', '一升パン', 104, 0xe2c89e, '風呂敷に包んだお祝いの一升パン。一歳の記念に。'),
  ...
  // --- South in North カフェメニュー (S1) ---
  food('spice_curry', 'スパイスカレー', 8, 0x9a5a24, '旧給食室で仕込むSouth in Northの名物スパイスカレー。'),
  food('quiche', 'キッシュ', 7, 0xe0a84a, '季節野菜の自家製キッシュ。サクサク生地がごちそう。'),
  food('tartine', 'タルティーヌ', 6, 0xe8c890, '日替わりタルティーヌ。いちごクリームチーズやあんバター、今日は何かな。'),
  food('veg_soup', '野菜スープ', 5, 0xe08830, '規格外野菜をことこと煮込んだ、もったいないを笑顔に変えるスープ。'),
  food('baked_sweets', '焼き菓子', 4, 0xcaa050, '酵母ビスコッティなどの素朴な焼き菓子。コーヒーのお供に。'),
  food('seasonal_drink', '季節のドリンク', 3, 0xc87840, '自家製コンフィチュールで作る季節のドリンク。'),

  // --- プチカンパーニュ食べ比べ (¥600・酵母の一期一会) ---
  food('petit_campagne_plain', 'プチカンパーニュ(プレーン)', 6, 0xc79a5b, '食べ比べの基本のプレーン。季節の酵母が織りなす一期一会の味。'),
  food('petit_campagne_raisin', 'プチカンパーニュ(レーズン)', 6, 0xa87848, 'レーズンの甘みと酵母の酸味を食べ比べ。今日だけの一期一会。'),
  food('petit_campagne_choco', 'プチカンパーニュ(チョコ)', 6, 0x6a4630, 'ほろ苦チョコと天然酵母の食べ比べ。出会いは一期一会。'),
  food('petit_campagne_walnut', 'プチカンパーニュ(クルミ)', 6, 0xb08a58, '香ばしいクルミ入り。酵母の違いを食べ比べる一期一会のひと粒。'),
];
