...
const SEGS = [
  ...
  { name: 'harvest', sec: 5.0, time: 0.4, // 校庭の畑で小麦・規格外野菜を採る
    place: [-4, 31, -0.5, 0], look: [[-0.5, -0.55], [0.2, -0.6], [0.7, -0.5]],
    mine: true,
    before: () => { window.__time(0.4); } },
  ...
  { name: 'queue', sec: 5.0, time: 0.55, // 開店の行列を外から（モブの歩きも決定的ステップで滑らか）
    place: [4, 31, -18, -0.5], look: [[-0.5, -0.02], [0.1, -0.04], [0.5, -0.02]] },
...
// デバッグFPS表示だけ隠す（インラインstyleで毎フレーム再表示されてもCSSが勝つよう両方）
await page.addStyleTag({ content: '#stats{display:none!important}' });
await page.evaluate(() => { const el = document.getElementById('stats'); if (el) el.style.setProperty('display', 'none', 'important'); });
...
for (const s of segs) {
  ...
  // チャンク生成を実時間ループで進める（rAFは手動なので step で回す）
  for (let i = 0; i < 40; i++) { await page.evaluate(ms => window.__cap.step(ms), 50); await page.waitForTimeout(25); }
  ...
  for (let i = 0; i < frames; i++) {
    ...
    if (s.at) for (const k of Object.keys(s.at)) {
      if (t >= Number(k) && !fired.has(k)) { fired.add(k); await page.evaluate(s.at[k]); }
    }
    // カメラ
    if (s.fly) {
      const c = lerpKeys(s.fly, t);
      await page.evaluate(a => window.__view(...a), c);
    } else if (s.look) {
      const [yaw, pitch] = lerpKeys(s.look, t);
      await page.evaluate(a => window.__look(a[0], a[1]), [yaw, pitch]);
    }
    // 採取: 一定間隔でクリック（クリエイティブ=即破壊）
    if (s.mine && i % Math.round(FPS * 0.9) === Math.round(FPS * 0.4)) {
      await page.mouse.down(); await page.evaluate(ms => window.__cap.step(ms), STEP_MS); await page.mouse.up();
    }
    await page.evaluate(([ms, time]) => { window.__time(time); window.__cap.step(ms); }, [STEP_MS, s.time]);
